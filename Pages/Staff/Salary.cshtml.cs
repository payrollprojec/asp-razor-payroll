...
namespace PayrollAppRazorPages.Pages.Staff
{
    ...
    public class SalaryModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            StaffSalary = await _context.StaffSalary.Include(ss => ss.StaffSalaryExtras).ThenInclude(extra => extra.SalaryItem).Where(ss => ss.staffID == applicationUser.Id && ss.Year == int.Parse(SelectedYear)).ToListAsync();
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                foreach (var s in StaffSalary)
                {
                    if (s.Month == i + 1)
                    {
                        ...
                        foreach (var j in s.StaffSalaryExtras)
                        {
                        ...
                        decimal net = s.BasicSalary- s.Absent+extraEarns-extraDucts + s.Allowances + s.Bonus + s.AdvSalaryPlus - s.EPF - s.SocsoRm - s.EIS - s.Tax - s.AdvSalary;
